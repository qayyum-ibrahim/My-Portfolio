...
import { ArrowUpRight, Github, ExternalLink, Layers, Server, Layout, Database, Shield } from "lucide-react";
...
const projects = [
  ...
  },
  {
    ...
    type: "System Design",
    details: "Implemented strict data isolation using row-level security and separate schemas. Designed the onboarding flow that automatically provisions resources.",
    links: { github: "#" }
  },
  {
    id: "payment-ledger",
    ...
    type: "Backend Core",
    details: "Designed a ledger system that guarantees zero-sum balance across all accounts, handling race conditions during high-volume transfers.",
    links: { github: "#" }
  },
  {
    id: "job-queue",
    ...
    type: "Infrastructure",
    details: "Decoupled heavy processing from the main API, improving response times by 80%. Implemented exponential backoff for failed jobs.",
    links: { github: "#" }
  },
  {
    id: "dashboard-ui",
  ...
  },
  {
    ...
    category: "frontend",
    description: "Conversion-optimized landing page with smooth Framer Motion animations.",
  ...
  return (
    <div className="min-h-screen py-20">
      <div className="container mx-auto px-6">
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-display font-bold mb-4">Selected Projects</h1>
          <p className="text-zinc-400 max-w-2xl">
            A collection of systems, architectures, and interfaces I've built. 
            Ranging from high-scale backend services to polished user interfaces.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                <div className="p-8 md:p-12 flex flex-col justify-center">
                  <div className="flex items-center gap-2 mb-4 text-blue-400 font-mono text-xs uppercase tracking-wider">
                    <Badge variant="outline" className="border-blue-500/30 text-blue-400">Flagship Product</Badge>
                    <span>Production System</span>
                  </div>
                  
                  <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">Univibe</h2>
                  <p className="text-zinc-400 mb-6 leading-relaxed">
                    A comprehensive event ticketing platform built to handle high-concurrency demands. 
                    Unlike simple CRUD apps, Univibe handles complex state management for seat reservations, 
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    {projects[0].tech.map(t => (
                      <span key={t} className="px-2 py-1 bg-zinc-950 border border-zinc-800 rounded text-xs text-zinc-400 font-mono">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="bg-zinc-950 border-l border-zinc-800 min-h-[400px] flex items-center justify-center relative p-8">
                  ...
                  <div className="relative w-full max-w-md aspect-video border border-zinc-800 bg-zinc-900/50 rounded-lg flex items-center justify-center">
                    <span className="text-zinc-600 font-mono text-sm">Product UI / Dashboard Preview</span>
                    {/* Placeholder for actual screenshot */}
                    <div className="absolute top-4 left-4 right-4 h-4 bg-zinc-800 rounded-sm" />
                    <div className="absolute top-12 left-4 w-1/4 h-32 bg-zinc-800 rounded-sm" />
                    <div className="absolute top-12 right-4 w-2/3 h-32 bg-zinc-800 rounded-sm" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.filter(p => p.id !== "univibe").map((project) => (
            <motion.div
              key={project.id}
              ...
              <Card className="h-full bg-zinc-900/20 border-zinc-800 hover:border-zinc-700 transition-colors group">
                <CardHeader>
                  <div className="flex justify-between items-start mb-2">
                    <div className="p-2 bg-zinc-950 rounded border border-zinc-800 text-zinc-400 group-hover:text-white transition-colors">
                      {project.category === "architecture" ? <Server className="w-5 h-5" /> : <Layout className="w-5 h-5" />}
                    </div>
                ...
                <CardContent>
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {project.tech.map(t => (
                      <Badge key={t} variant="secondary" className="bg-zinc-800/50 text-zinc-400 hover:bg-zinc-800 border-transparent text-[10px]">
                        {t}
                      </Badge>
                    ))}
                  </div>
